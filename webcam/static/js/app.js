  ...
  methods: {
    ...
    toggleRecord() {
      ...
      if (!this.isRecording) {
        ...
        // return false
        // that.oneTimer = setInterval(() => {
        //   that.endTime = that.endTime - 1
        //   console.log("倒计时1111", that.endTime)
        //   if (that.endTime == 0 || that.endTimeTwo == 0) {
        ...
        //     that.twoTimer = setInterval(() => {
        //       // alert('显示',this.seconds)
        //       that.seconds = this.seconds - 1
        //       // this.times--;
        //       if (that.seconds == 0) {
        //         clearInterval(that.twoTimer)
        //         clearInterval(that.oneTimer)
        //         that.beginVideoPlay()
    ...
    beginVideoPlay() {
      ...
      if (!that.mediaRecorder || that.mediaRecorder.state === "inactive") {
        that.recordedBlobs = []
        that.mediaRecorder = new MediaRecorder(that.mirrorStream, {
          // ✅ 使用 mirrorStream
          mimeType: "video/mp4"
        })

        that.mediaRecorder.ondataavailable = (event) => {
        ...
        that.mediaRecorder.onstop = () => {
          ...
          that.urlFuzhi(url) // 你可以在这里设置视频预览或上传
        }
    ...
    urlFuzhi(url) {
      ...
      if (!that.authorizeLz) {
        ...
        that.tipsLabel.forEach((label, index) => {
          ...
          option.addEventListener("click", () => {
            ...
            that.updateSubmitButtonState() // 更新提交按钮状态
          })
        ...
        // 初始化时检查一次按钮状态
        that.updateSubmitButtonState()
      } else {
        that.videoTwo = url
        that.checkDoctorAuth(that.doctorId, that.userName)
    ...
    stopCamera() {
      let that = this
      if (that.stream) {
        const tracks = that.stream.getTracks()
        tracks.forEach((track) => track.stop())
        that.stream = null
        // that.videoSrc = null;
      }
      // that.showWebcam = false;
    },
    updateSubmitButtonState() {
      ...
      if (selectedCount === that.tipsLabel.length) {
        console.log("全部")
        that.isSubmit = true
